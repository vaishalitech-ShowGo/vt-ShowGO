    ...
    <nav className="fixed top-0 left-0 w-full h-[99px] bg-black flex items-center justify-between md:justify-around md:px-16 z-50 shadow-lg">
      ...
      {isLoggedIn ? (
        <div className="relative">
          <img
            src="https://images.pexels.com/photos/1586663/pexels-photo-1586663.jpeg"
            alt="User Avatar"
      ...
      {isOpen && (
        <div className="absolute top-[99px] left-0 w-full bg-black text-white flex flex-col items-center py-5 md:hidden">
          {["home", "events", "about"].map((tab) => (
            <Link key={tab} to={`/${tab === "home" ? "" : tab}`} onClick={() => setIsOpen(false)}>
              <button
          ...
          {!isLoggedIn && (
            <Link to="/login-signup">
              <button
                className="bg-white text-black w-[149.26px] h-[40px] rounded-full font-semibold shadow-lg hover:opacity-80 flex items-center justify-center cursor-pointer mt-4"
